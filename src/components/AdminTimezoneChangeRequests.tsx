...
export function AdminTimezoneChangeRequests() {
  const [statusFilter, setStatusFilter] = useState<"open" | "approved" | "rejected" | "all">("open");
  const [requests, setRequests] = useState<TimezoneRequest[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    const res = await fetch(`/api/admin/timezone-requests?status=${statusFilter}&limit=50`);
    setLoading(false);
    if (!res.ok) {
      setError("Failed to load timezone requests");
      return;
    }
  ...
  async function resolveRequest(id: string, status: "approved" | "rejected") {
    setError(null);
    const res = await fetch(`/api/admin/timezone-requests/${id}`, {
      method: "PATCH",
  ...
  return (
    <section className="card p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-medium">Timezone change requests</h2>
          <p className="text-sm text-muted">
            Users request a new timezone for visit times and alerts. Approve to update their
            account.
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                ...
                <p className="mt-1 text-muted">
                  {timezoneLabel(r.currentTimezone)} → {timezoneLabel(r.requestedTimezone)}
                </p>
                <p className="mt-1 text-xs text-muted">
                  Active timezone: <code className="text-accent">{r.user.timezone}</code>
                </p>
