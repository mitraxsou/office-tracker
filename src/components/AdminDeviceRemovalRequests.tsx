...
type RemovalRequest = {
  ...
  device: { id: string; serialNumber: string; lastSeenAt: string | null };
  resolvedByEmail: string | null;
};

export function AdminDeviceRemovalRequests() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/admin/device-removal-requests?status=${statusFilter}&limit=50`,
    );
    setLoading(false);
    if (!res.ok) {
  ...
  async function resolveRequest(id: string, status: "approved" | "rejected") {
    setError(null);
    const res = await fetch("/api/admin/device-removal-requests", {
      method: "PATCH",
  ...
  return (
    <section className="card p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-medium">Laptop removal requests</h2>
          <p className="text-sm text-muted">
            Users request removal to avoid accidental de-registration. Approve to remove the device
            from their account.
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                ...
                <p className="mt-1 text-muted">
                  Laptop <code className="text-accent">{r.device.serialNumber}</code>
                  {r.device.lastSeenAt && (
                    <span className="ml-2">
                      · last seen {new Date(r.device.lastSeenAt).toLocaleString("en-IN")}
                    </span>
                  )}
                </p>
                ...
                <p className="mt-1 text-xs text-muted">
                  ...
                  {r.resolvedAt && (
                    <>
                      {" "}
                      · {r.status} {new Date(r.resolvedAt).toLocaleString("en-IN")}
                ...
                {resolvingId === r.id ? (
                  <div className="space-y-2">
                    <textarea
                      ...
                      placeholder="Optional note to user (internal)"
                      rows={2}
